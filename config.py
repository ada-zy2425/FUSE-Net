...
def get_config(parse=True, **optional_kwargs):
    ...
    # 在 get_config(parse=True, ...) 里的 argparse 增加：
    p.add_argument('--use_fusenetpp', type=str2bool, default=False)
    p.add_argument('--use_probe',     type=str2bool, default=False)
    p.add_argument('--lambda_domain', type=float,    default=0.0)   # 先为 0，不生效
    p.add_argument('--domain_mode',   type=str,      default='mmd') # mmd/coral/contrastive
    p.add_argument('--mmd_gamma',     type=float,    default=1.0)
    p.add_argument('--coral_gamma',   type=float,    default=1.0)   
